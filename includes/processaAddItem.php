...
if(isset($_POST['add-Item']) && $_POST['add-Item'] == 'true'):
	...
	var sound = new Howl({
        ...
        elseif(!empty($_POST['sabormeioameio']) && !empty($_POST['meio_a_meio_tipo']) && $_POST['meio_a_meio_tipo'] == 3):
    		...
    		//$maiorvalor = max($valorsabores);
    		$mediavalor = 0;
    		for ($i = 0; $i < count($valorsabores); $i++) {
    		    $mediavalor = $mediavalor + $valorsabores[$i];
    		}
    		$mediavalor = $mediavalor / count($valorsabores);
    
    		$valorfinalsabores = (!empty($mediavalor) ? $mediavalor : 0);
    		$nomesfinalsabores = implode(" + ", $nomesabores);
    		$nomesfinalsabores = $nomesfinalsabores;
...
if(!empty($_POST['observacao'])):
	if(is_array($_POST['observacao'])):
		$getObservacao = implode(", ", $_POST['observacao']);
	else:
		$getObservacao = strip_tags(trim($_POST['observacao']));
	endif;
else:
	$getObservacao = "Não";
endif;
...
if(!empty($_POST['adicionalGratis'])):
	$detalhes_pedido_adicionais_gratis = implode(', ', $_POST['adicionalGratis']);	
	$detalhes_pedido['adicionais_gratis'] = $detalhes_pedido_adicionais_gratis;			
endif;


if(!empty($_POST['opcao_unica'])):
	$detalhes_pedido['opcao_unica'] = $_POST['opcao_unica'];			
endif;
