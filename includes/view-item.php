...
//INICIO PAGINAÇÃO
              ...
              if(!empty($search_url)):
                ...
                $quary   = "WHERE user_id = :userid AND (nome_item LIKE '%' :linkum '%' or descricao_item LIKE '%' :linkdois '%') ";
                $quary2  = "userid={$userlogin['user_id']}&linkum={$search_url}&linkdois={$search_url}&";
...
//FIM PAGINAÇÃO
              ...
              $lerbanco->ExeRead("ws_itens", "{$quary}ORDER BY id_cat ASC, nome_item ASC LIMIT :limit OFFSET :offset", "{$quary2}limit={$pager->getLimit()}&offset={$pager->getOffset()}"); 
              ...
              <td><input type="number" data-produtoid="<?=$id;?>" class="form-control numero posicao" name="id" min="1" max="1000" value="<?=(!empty($posicao) ? $posicao : "")?>" placeholder="0">
              </td>
...
<script type="text/javascript">
  $('.posicao').change(function (){
    ...
    $.ajax({
      url: '<?=$site;?>controlers/edit-opcao-posicao.php',
      method: "post",
