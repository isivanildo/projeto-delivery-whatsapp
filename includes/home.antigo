...
if($lerbanco->getResult()):
	...
	foreach ($lerbanco->getResult() as $adicionalarray):
		...
		$checkopsabor = ($id == $ido_DoItem ? "checked readonly" : "");
		
		echo "<label>
		<input type=\"checkbox\" {$checkopsabor} class=\"id_checkbox{$id}\" class=\"controlecheckbox\" name=\"sabormeioameio[]\" value=\"{$nome_item}-_-{$preco_item}\" /> {$nome_item} 
