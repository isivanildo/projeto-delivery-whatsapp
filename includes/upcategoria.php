...
if($capturacatup && isset($capturacatup['sendupdatecategoria'])):
  ...
  $capturacatup['desc_cat'] = strip_tags(trim($capturacatup['desc_cat']));
  $capturacatup['nome_cat'] = strip_tags(trim($capturacatup['nome_cat']));
  $capturacatup['dias_semana'] = (!empty($capturacatup['dias_semana']) ? implode(",", $capturacatup['dias_semana'])  : "null");
  ...
  if($capturacatup['nome_cat'] == ""):
    ...
    header("Refresh: 3; url={$site}{$Url[0]}/upcategoria&id={$getIdCat}#upcat");
  else:
...
<div class="form-group">
 ...
 <div class="col-sm-10">
   <div id="myselect">
    <select class="form-control" id="multiselect" name="dias_semana[]" multiple="multiple">
     <?php
     if($dias_semana == "null"):
      ?>
      <option selected value="Domingo"> Domingo</option>
      <option selected value="Segunda"> Segunda</option>
      <option selected value="Terça"> Terça</option>
      <option selected value="Quarta"> Quarta</option>
      <option selected value="Quinta"> Quinta</option>
      <option selected value="Sexta"> Sexta</option>
      <option selected value="Sabado"> Sabado</option>
      <?php
    else:
     $exp = explode(',', $dias_semana);
     $Domingo = (in_array("Domingo", $exp) ? "selected" : "");
     $Segunda = (in_array("Segunda", $exp) ? "selected" : "");
     $Terca = (in_array("Terça", $exp) ? "selected" : "");
     $Quarta = (in_array("Quarta", $exp) ? "selected" : "");
     $Quinta = (in_array("Quinta", $exp) ? "selected" : "");
     $Sexta = (in_array("Sexta", $exp) ? "selected" : "");
     $Sabado = (in_array("Sabado", $exp) ? "selected" : "");


     echo "
     <option {$Domingo} value=\"Domingo\"> Domingo</option>
     <option {$Segunda} value=\"Segunda\"> Segunda</option>
     <option {$Terca} value=\"Terça\"> Terça</option>
     <option {$Quarta} value=\"Quarta\"> Quarta</option>
     <option {$Quinta} value=\"Quinta\"> Quinta</option>
     <option {$Sexta} value=\"Sexta\"> Sexta</option>
     <option {$Sabado} value=\"Sabado\"> Sabado</option>    
     ";
