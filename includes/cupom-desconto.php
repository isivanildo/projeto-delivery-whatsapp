          ...
          <form id="formcupom" method="post">
            <div class="row">
              <div class="col-md-12 col-sm-12">
                <div class="form-group">
                    <span id="spanShowCollapse" data-toggle="collapse" data-target="#demo"></span>
                    <label for="exampleSelectTypeDiscount">Tipo de Desconto</label>
                    <select class="form-control" name="type_discount" required>
                        <option value="-1">Selecione o tipo de desconto</option>
            ...
            <div class="collapse" id="demo">
                <div class="row">
                  ...
                  <div class="col-md-6 col-sm-6">
                    <div class="form-group" id="content_cupom">
...
<script type="text/javascript">
  $('select').on('change', function() {
    if (this.value == 1) {
        $("#content_cupom").html("<label for=\"exampleInputPassword1\">Desconto %</label>"+
                  "<input required type=\"number\" class=\"form-control descontoporcentagem\" value=\"1\" name=\"porcentagem\" min=\"1\" max=\"100\" />"+
                  "<small class=\"form-text text-muted\">Porcentagem de desconto.</small>");
        
        if ($("#demo").attr("class").indexOf("in") == -1) {
            $("#spanShowCollapse").click();
        }
    }
    else if (this.value == 2) {
        $("#content_cupom").html("<label for=\"exampleInputPassword1\">Desconto R$</label>"+
                  "<input required type=\"text\" class=\"form-control\" value=\"1.00\" name=\"fixed_value\" />"+
                  "<small class=\"form-text text-muted\">Valor de desconto.</small>");
        if ($("#demo").attr("class").indexOf("in") == -1) {
            $("#spanShowCollapse").click();
        }
    }
