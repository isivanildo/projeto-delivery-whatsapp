...
if(empty($inputdadosempresa['access_token_mp'])):
	unset($inputdadosempresa['access_token_mp']);
endif;

if(empty($inputdadosempresa['token_pagseguro'])):
	unset($inputdadosempresa['token_pagseguro']);
endif;

if(empty($inputdadosempresa['email_pagseguro'])):
	unset($inputdadosempresa['email_pagseguro']);
endif;

						// FIM DA VALIDAÇÃO BACKGROUND 
						//---------------------------
						

						// INICIO DA VALIDAÇÃO BACKGROUND
		if (isset($_FILES['img_fundo']['tmp_name']) && $_FILES['img_fundo']['tmp_name'] != ""):
			$inputdadosempresa['img_fundo'] = $_FILES['img_fundo'];
		else:
			unset($inputdadosempresa['img_fundo']);
		endif;


		if(!empty($inputdadosempresa['img_fundo'])):                        
			$upload = new Upload("uploads/");
			$upload->Image($inputdadosempresa['img_fundo']);

			if (isset($upload) && $upload->getResult()):	

				$inputdadosempresa['img_fundo'] = $upload->getResult();

		elseif(is_array($inputdadosempresa['img_fundo'])):
			unset($inputdadosempresa['img_fundo']);
		endif;

		if(!empty($inputdadosempresa['img_fundo']) && !empty($img_fundo) && file_exists("uploads/{$img_fundo}") && !is_dir("uploads/{$img_fundo}")):
			unlink("uploads/{$img_fundo}");
	endif;						
...
elseif (!Check::Email($inputdadosempresa['email_empresa'])):
	...
	//Ivanildo Ferreira 06/09/2021
	$inputdadosempresa['facebook_empresa'] = (!empty($inputdadosempresa['facebook_empresa']) ? $inputdadosempresa['facebook_empresa'] : "");
	$inputdadosempresa['instagram_empresa'] = (!empty($inputdadosempresa['instagram_empresa']) ? $inputdadosempresa['instagram_empresa'] : "");
	$inputdadosempresa['twitter_empresa'] = (!empty($inputdadosempresa['twitter_empresa']) ? $inputdadosempresa['twitter_empresa'] : "");
	$inputdadosempresa['access_token_mp'] = (!empty($inputdadosempresa['access_token_mp']) ? $inputdadosempresa['access_token_mp'] : "");
	$inputdadosempresa['token_pagseguro'] = (!empty($inputdadosempresa['token_pagseguro']) ? $inputdadosempresa['token_pagseguro'] : "");
	$inputdadosempresa['email_pagseguro'] = (!empty($inputdadosempresa['email_pagseguro']) ? $inputdadosempresa['email_pagseguro'] : "");
	//Fim
...
<div class="form-group">
	<label class="form-check-label" for="btn_whats">Estado do Checkbox "Enviar pedido pelo whatsapp"</label>
	<select required class="form-control" name="btn_whats" id="btn_whats">
		<option value="0" <?=($btn_whats == 0 ? 'selected' : '');?>>Previamente selecionado</option>
		<option value="1" <?=($btn_whats == 1 ? 'selected' : '');?>>Desativado</option>
	</select>
</div>
...
<div class="panel-group" id="accordion">
	...
	<div class="wrapper_indent add_bottom_45">
		...
		<div class="form-group">
			<label>Imagem de perfil, será redimensionada em 240 X 240:</label>
			<div class="input-file-container">  
				<input name="img_fundo" class="input-file" id="my-file" type="file" />
				<label tabindex="0" for="my-file" class="input-file-trigger">Enviar Imagem...</label>
			</div>
			<p class="file-return"></p>
			<br />
