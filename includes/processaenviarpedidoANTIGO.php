...
if(isset($get_dados_pedido['enviar_pedido']) && $get_dados_pedido['enviar_pedido'] == 'enviar_agora'):
	...
	elseif(!empty($get_dados_pedido['forma_pagamento'])):
		...
		$quarta_parte_pedido = "*Pagamento:* {$get_dados_pedido['forma_pagamento']}<br />*SubTotal* R$ ".Check::Real($get_dados_pedido['sub_total'])."<br />".($_SESSION['desconto_cupom']['type_discount'] == 1 ? "{$porcentagemg}" : "{$fixed_valueg}")."{$msgSedelivery}*Total:* {$moeda} {$get_dados_pedido['total']}<br />*Troco para:* {$moeda} {$get_dados_pedido['valor_troco']}<br /><br />*{$get_dados_pedido['data']}*";
...
if($store_hours->is_open()) {
	...
	if($get_dados_pedido['opcao_delivery'] == 'true' && !empty($minimo_delivery) && $minimo_delivery != '0.00' && $get_dados_pedido['total'] < $minimo_delivery):
		...
		echo "<script>
		x0p('', 
		'Opss... O valor mínimo do delivery e de R$ {$maco} ~ {$minimo_delivery} ~ {$get_dados_pedido['total']}',
		'error', false);
